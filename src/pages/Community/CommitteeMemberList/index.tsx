...
export default () => {
  ...
  const confirmTemplate = () => {
    ...
    FormRef.validateFields()
      .then(async () => {
        ...
        addCommitteeMember(response).then((res) => {
          console.log('add', res);
        });
        // 重置输入的表单
        FormRef.resetFields();
  ...
  const toDelContractTemplate = async (record) => {
    ...
    let { serialNo } = record;
    let delRes = await deleteCommitteeMember({ serialNo });
  ...
  const toViewContractTemplate = async (record) => {
    let { serialNo } = record;
    let viewRes = await getCommitteeMemberDetail({ serialNo });
  ...
  return (
    <div>
      ...
      <ProTable<columnsDataType>
        headerTitle={<TableTitle title="社区委员会成员" />}
        scroll={{ x: 900 }}
        bordered
        // 表头
        columns={columns}
        ...
        request={async (params) => {
          // console.log(params);
          let res = await getCommitteeMemberPageList({
            ...params,
