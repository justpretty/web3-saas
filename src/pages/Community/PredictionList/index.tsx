...
export default () => {
  ...
  const confirmTemplate = () => {
    ...
    FormRef.validateFields()
      .then(async () => {
        ...
        addPrediction(response).then((res) => {
          console.log('add', res);
        });
        // 重置输入的表单
        FormRef.resetFields();
  ...
  const toDelContractTemplate = async (record) => {
    ...
    let { serialNo } = record;
    let delRes = await deletePrediction({ serialNo });
  ...
  const toViewContractTemplate = async (record) => {
    let { serialNo } = record;
    let viewRes = await getPredictionDetail({ serialNo });
  ...
  return (
    <div>
      ...
      <ProTable<columnsDataType>
        headerTitle={<TableTitle title="预测列表" />}
        scroll={{ x: 900 }}
        bordered
        // 表头
        columns={columns}
        ...
        request={async (params) => {
          // console.log(params);
          let res = await getPredictionPageList({
            ...params,
