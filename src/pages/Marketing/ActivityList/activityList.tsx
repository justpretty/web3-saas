...
export default ({ setIsLoadAddActivity }) => {
  ...
  const toDelContractTemplate = async (record) => {
    ...
    let { serialNo } = record;
    let delRes = await deleteActivity({ serialNo });
  ...
  const toViewContractTemplate = async (record) => {
    let { serialNo } = record;
    let viewRes = await getActivityDetail({ serialNo });
  ...
  return (
    <div>
      ...
      <ProTable<columnsDataType>
        headerTitle={<TableTitle title="活动列表" />}
        scroll={{ x: 900 }}
        bordered
        // 表头
        columns={columns}
        ...
        request={async (params) => {
          // console.log(params);
          let res = await getActivityPageList({
            ...params,
