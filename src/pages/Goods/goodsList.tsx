...
export default ({ setIsLoadAddGoods }) => {
  ...
  const toDelContractTemplate = async (record) => {
    ...
    let { serialNo } = record;
    let delRes = await deleteGoods({ serialNo });
  ...
  const toViewContractTemplate = async (record) => {
    let { serialNo } = record;
    let viewRes = await getGoodsDetail({ serialNo });
  ...
  return (
    <div>
      ...
      <ProTable<columnsDataType>
        headerTitle={<TableTitle title="商品列表" />}
        scroll={{ x: 900 }}
        bordered
        // 表头
        columns={columns}
        ...
        request={async (params) => {
          // console.log(params);
          let res = await getGoodsPageList({
            ...params,
